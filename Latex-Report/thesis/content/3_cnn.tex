...
We observe from the table that for the first convolution layer, the execution time in Intel Core i3 CPU is 12.308 ms for 300,020 MAC operations. Hence, we infer that one MAC operation takes around $\frac{12.308 ms}{300020}$ = 41 ns.  Similarly, one MAC operation in Zedboard is estimated to be $\frac{66.577 ms}{300020}$ = 221.9 ns. \newline \newline
Using these estimates, we assess the number of MAC operations per second as follows:
\begin{enumerate}
\item Intel Core i3-2350M:  $\frac{1}{41 ns}$ = 24,390,243.9 $\approx$ 24M MACs/second
\item Zedboard CPU:  $\frac{1}{221.9 ns}$ = 4,506,534.475 $\approx$ 4M MACs/second
\end{enumerate}
In the software implementation, the input image is loaded from external memory. Hence, loading image from external memory and storing intermediate results and output in the external memory can create a lot of overhead. Typically, this data movement overhead is overcome by storing the images on-chip, avoiding unnecessary load-stores in external memory. Doing so, we can heavily accelerate the convolution and inner product layers, achieving a greater speed-up.
...
\begin{enumerate}
\item Intel Core i3-2350M: $\frac{12.308 ms}{150 \mu s}$ = 82.05
\item Zedboard CPU: $\frac{66.577 ms}{150 \mu s}$ = 443.84
\end{enumerate}		
The hypothetical platform is approximately 82 times faster than the Zedboard ARM and 440 times faster than Intel platform under test.\\
Doing a similar analysis for \textbf{convolution layer 2}: \\
Each 12 $\times$ 12 input image involves 1689 MACs (Filtering: (8 $\times$ 8 $\times$ 5 $\times$ 5) + Feature Map Accumulation: (5 $\times$ 5) + Bias Addition: (8 $\times$ 8)). 
\begin{itemize}
...
\end{itemize}
With our hypothetical platform, the time taken for getting all outputs of Convolution Layer 2 is 57 times faster than Intel platform and 426 times faster than Zedboard ARM. 
...
\end{tikzpicture}
The total time required to classify one input image is 1.21ms in Intel Core i3-2350M, 14.526ms in NVIDIA GeForce 315M, 3.905ms in NVIDIA Quadro 600 and 322 us in Intel Xeon platform. This implies that in one second, the number of images that can be processed by each of these heterogeneous platforms is given by:
\begin{enumerate}
...
\end{enumerate}
We observe that with more memory to work on and more compute units, more threads can be concurrently executed and hence, more speedup can be achieved. 