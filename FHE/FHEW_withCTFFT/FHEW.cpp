...
void AddToACC_test(ct_FFT ACC, ct_FFT C) {
    ct_ModQ ct;
    dct_ModQ dct;
    dct_FFT dctFFT;
    // Decompose_ct(dctFFT, ACC);
...
//       cout << r << ",";
  ...
  void HomNAND(LWE::CipherText* res, const EvalKey& EK, const LWE::CipherText& ct1, const LWE::CipherText& ct2) {
   // homNand++;
	...
	{
	   if(i<10)
	      AddToACC_test(ACC, *(EK.BSkey[i][a0][k]));	
	   else
